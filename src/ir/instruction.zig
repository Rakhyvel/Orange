...
pub fn init_call(dest: *lval_.L_Value, src1: *lval_.L_Value, span: Span, allocator: std.mem.Allocator) *Self {
    ...
    retval.data = Data{ .call = .{
        ...
        .ability_arg_lval_list = std.array_list.Managed(*lval_.L_Value).init(allocator),
    } };
    return retval;
}
...
pub fn pprint(self: Self, allocator: std.mem.Allocator) ![]const u8 {
    ...
    switch (self.kind) {
        ...
        .call => {
            ...
            try print_lval_list(self.data.call.arg_lval_list, &out); // TODO: print ability args, too
            try out.print(")\n", .{});
        },
        .invoke => {
            try out.print("    {f} := ::{s}(", .{ self.dest.?, self.data.invoke.method_decl.method_decl.name.token().data });
            try print_lval_list(self.data.invoke.arg_lval_list, &out); // TODO: print ability args, too
            try out.print(")\n", .{});
        },
        .push_stack_trace => {
...
pub const Data = union(enum) {
    ...
    call: struct {
        ...
        ability_arg_lval_list: std.array_list.Managed(*lval_.L_Value),
    },
    invoke: struct {
        ...
        ability_arg_lval_list: std.array_list.Managed(*lval_.L_Value), // List of args. Receiver will always be prepended
